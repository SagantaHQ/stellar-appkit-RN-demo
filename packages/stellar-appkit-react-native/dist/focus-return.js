 ...
 */
export function resolveAppFocusTarget(redirect) {
    if (!redirect)
        return null;
 ...
 */
export function attachAppFocusReturn(client) {
    ...
    const attempt = () => {
        const now = Date.now();
        if (now - lastAttemptAt < FOCUS_ATTEMPT_COOLDOWN_MS)
            return;
        const target = resolveAppFocusTarget(client.appMetadata?.redirect);
        if (!target || !shouldAttemptAppFocus({ appState: AppState.currentState, target }))
            return;
        lastAttemptAt = now;
 ...
 */
export function useAppFocusReturn(client) {
    useEffect(() => attachAppFocusReturn(client), [client]);
}
//# sourceMappingURL=focus-return.js.map