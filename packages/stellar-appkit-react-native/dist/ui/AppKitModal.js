...
export function AppKitModal({ client, open, onClose, theme = defaultTheme, mode = 'bottomsheet', title, logo, browser, autoCloseOnComplete = true, autoOpenWalletOnSign = true, }) {
    ...
    const armSignHandoff = useCallback(() => {
        if (!autoOpenRef.current)
            return; // disabled — manual button only
    ...
    useEffect(() => {
        const setter = wcConnector?.setOnSignRequestDispatch;
        if (typeof setter !== 'function')
            return; // older core — manual handoff only
    ...
    const signHandoffWalletId = useMemo(() => resolveSignHandoffWalletId(wcConnector?.getSessionPeer?.()?.redirect ?? null, pairedMobileWalletId.current), 
     ...
     */
    const reopenPairedWallet = useCallback(async (targetId) => {
    ...
    const body = (_jsxs(_Fragment, { children: [view === 'list' && (_jsx(WalletListView, { styles: styles, theme: theme, loading: loadingWallets, rows: walletRows, showMobileWallets: Boolean(wcConnector), showMore: showMoreWallets, onToggleMore: () => setShowMoreWallets((v) => !v), onConnectMobile: connectMobileWallet, onConnectConnector: connectConnector, onInstall: (row) => {
                ...
                onOpenWallet: signHandoffWalletId ? () => void reopenPairedWallet(signHandoffWalletId) : signHandoffLink ? reopenSignHandoffWallet : undefined, onRetry: () => {
