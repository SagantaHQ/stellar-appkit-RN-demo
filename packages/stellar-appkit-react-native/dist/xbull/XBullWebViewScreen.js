 ...
 * ```
 */
export function createXBullWebViewBridge(render) {
    return {
        openWallet(url, handlers) {
            return new Promise((resolve) => {
                render(_jsx(XBullWebViewScreen, { url: url, onMessage: handlers.onMessage, onClosed: handlers.onClosed, onReady: resolve, onUnmount: () => render(null) }));
            });
        },
    };
}
export function XBullWebViewScreen({ url, onMessage, onClosed, onReady, onUnmount, dark = true, }) {
    ...
    const dismiss = useCallback((byUser) => {
        ...
        if (webviewRef.current && !handlePublished.current) {
            ...
            onReady({
                postMessageToWallet: (msg, origin) => {
                    ...
                    webviewRef.current?.injectJavaScript(`window.dispatchEvent(new MessageEvent('message', { data: ${payload}, origin: ${JSON.stringify(origin)} })); true;`);
                },
                close: () => dismiss(false),
            });
        }
