...
export class FailoverRpcServer {
    ...
    getStatus() {
        return this.health.map((h) => ({
            url: h.server.serverURL?.toString() ?? 'unknown',
    ...
    pickServer() {
        ...
        throw new Error('All RPC servers are unhealthy. Last failure counts: ' +
            this.health.map((h) => `${h.server.serverURL?.toString() ?? '?'}=${h.failureCount}`).join(', '));
    }
    /**
     ...
     */
    async runWithFailover(method, args, isFailoverable) {
     ...
     */
    static isFailoverableError(err) {
        ...
        if (err && typeof err === 'object') {
            ...
            if (typeof e.code === 'number' && e.code === -32603)
                return true; // JSON-RPC internal error
        }
        return false;
    }
    /**
    ...
    get handler() {
        return {
            get: (target, prop, receiver) => {
                ...
                const value = Reflect.get(firstServer, prop);
                ...
                return (...args) => {
                    return target.runWithFailover(String(prop), args, FailoverRpcServer.isFailoverableError);
                };
            },
        };
    }
 ...
 */
export async function createFailoverRpcServer(urls, opts) {
    const { Server } = await import('@stellar/stellar-sdk/rpc');
    const servers = urls.map((url) => new Server(url));
    return new FailoverRpcServer({ servers, ...opts });
}
//# sourceMappingURL=rpc-failover.js.map