...
| After the wallet approves: `WARN requiredNamespaces are deprecated…` then `ERROR request() -> isValidRequest() failed` and `ERROR Missing or invalid. request() method: stellar_getNetwork` | **Fixed in this repo.** `@walletconnect/sign-client` ≥ 2.17 deprecates `requiredNamespaces` (auto-merges them into `optionalNamespaces` with a WARN) and validates every `request()` method against the methods the wallet actually approved — logging ERROR before throwing. Freighter Mobile implements only the four documented Stellar WC methods (`stellar_signXDR`/`stellar_signAndSubmitXDR`/`stellar_signMessage`/`stellar_signAuthEntry` — see the [Freighter Mobile WC docs](https://docs.freighter.app/mobile-walletconnect)), so the old connector's unconditional `stellar_getNetwork` request always failed validation (it silently fell back to the app's configured network, but the ERROR logs were scary). The vendored core now proposes `optionalNamespaces` only — full method set including `stellar_getNetwork` for wallets that do support it — and pre-checks every request against the session's approved methods: `stellar_getNetwork` is only sent when granted, and sign requests fail fast with a clean error instead of namespace-validation noise. |
